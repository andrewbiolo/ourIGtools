...
public class SAOnlyIterator implements IELSAIterator {
	...
	public SAOnlyIterator (int[] sa, B3LLSequence seq, int k){
		this.sa = sa;
		this.seq = seq;
		this.k = k;
	...
	public boolean next() {
		...
		int i;
		boolean found = false, ctrl = false;
		...
		if (this.deep >= 0) { // Checks if its not the root interval
		...
		while (found == false && this.deep >= 0) { 
			for (i = this.deep; i < this.k; i ++) { // Searching for a k-mer
				...
				if (index >= this.seq.length) { // Reached the end of the suffix array
					i = this.k + 1; // Break for condition, exit
					this.deep = -1;
				} else {
					byte current = this.getB3Safe(this.sa[this.limits[0][i]]+i);
					if (current != B3Nucleotide.NULL_CODE && current != B3Nucleotide.N_CODE) { // Get current interval
						...
						} else { // Refine interval
							this.limits[0][i+1] = this.limits[0][i];
							this.limits[1][i+1] = l - 1;
							this.deep ++;
						}
					} else {
						if (current == B3Nucleotide.NULL_CODE) { // Suffix shorter than k characters
							this.limits[0][i] = this.limits[0][i] + 1;
							if (this.limits[0][i] > this.limits[1][i]) this.limits[1][i] = this.limits[0][i];
							i = this.k + 1; // Break for condition, skip suffix
						} else { // A k-mer mustn't contain 'N'
							i = this.k + 1; // Break for condition, skip suffix
							this.deep --;
							if (this.deep >= 0) { // Skip 'N' series
								while (this.limits[1][this.deep] - this.limits[1][this.deep+1] <= 0) this.deep --;
								this.limits[0][this.deep] = this.limits[1][this.deep+1] + 1;
							} 
						}
					}
				}
	...
	public IELSAIterator clone() {
		SAOnlyIterator tmp = new SAOnlyIterator (this.sa, this.seq, this.k);
		for (int i = 0; i <= k; i ++) {
